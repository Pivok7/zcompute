...
const sm = zcomp.shared_memory;

pub const editWrapper = struct {
    // Using variable outside the function
    const outside_var: u32 = 5;

    pub fn editFunc(data: []u8, shrd_mem: *const sm.SharedMemory) void {
...
pub fn main() !void {
    ...
    const data = &[_]sm.SharedMemory{
        try sm.SharedMemory.Buffer.newSlice(
        ...
        try sm.SharedMemory.Buffer.newEmpty(i32, 10, 3),
    };
